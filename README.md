...
* Check the `ConnectionStrings` in `appsettings.json` files under the `DineProX.HttpApi.Host` and `DineProX.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 29ad44c1-ec30-4381-988a-5ea4240d6e66
```

> `29ad44c1-ec30-4381-988a-5ea4240d6e66` is the password of the certificate, you can change it to any password you want.
...
Run `DineProX.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `DineProX.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `DineProX.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
